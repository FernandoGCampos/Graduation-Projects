...
void convertsIndexFile1(char *binFileName){
    ...
    //Reads the root
    fread(&intBuffer, 4, 1, file);
    fprintf(newFile, "Raiz: %d\n", intBuffer);

    //Reads the next node
    fread(&intBuffer, 4, 1, file);
    fprintf(newFile, "Próximo nó: %d\n", intBuffer);

    //Reads the amount of nodes
    fread(&intBuffer, 4, 1, file);
    fprintf(newFile, "Quantidade de nós: %d\n", intBuffer);

    //Reads the trash in the header
    fprintf(newFile, "Lixo: ");
    for(int i = 13; i < 45; ++i){
        fread(&charBuffer, 1, 1, file);
        fprintf(newFile, "%c", charBuffer);
    ...
    while(fread(&charBuffer, 1, 1, file) != 0){
        fprintf(newFile, "RRN: %d\n", contador);
        fprintf(newFile, "Tipo: %c\n", charBuffer);

        //Reads the number of keys
        fread(&intBuffer, 4, 1, file);
        fprintf(newFile, "Número de chaves: %d\n", intBuffer);

        //Reads the values
        for(int i = 0; i < 3; ++i){
            fread(&intBuffer, 4, 1, file);
            fprintf(newFile, "ID: %d ", intBuffer);
            fread(&intBuffer, 4, 1, file);
            fprintf(newFile, "RRN dados: %d\n", intBuffer);
        }

        //Reads the childs
        for(int i = 0; i < 4; ++i){
            fread(&intBuffer, 4, 1, file);
            fprintf(newFile, "Filho%d: %d\n", i, intBuffer);
        }
        fprintf(newFile, "\n");
...
void convertsIndexFile2(char *binFileName){
    ...
    while(fread(&charBuffer, 1, 1, file) != 0){
        ...
        for(int i = 0; i < 3; ++i){
            ...
            fprintf(newFile, "ID: %d ", intBuffer);
            fread(&longBuffer, 8, 1, file);
            fprintf(newFile, "Posição dados: %ld\n", longBuffer);
        }
